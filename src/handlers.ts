...
export async function deleteFile(ctx: PluginContext, path: string, sessionId: string): Promise<{ path: string; deleted: true }> {
  ...
  try {
    ...
    if (info.isDirectory()) throw httpError(403, 'not-a-file', '只能删除文件，不能删除目录')
  } catch (error) {
  ...
  if (sessionId !== '') {
    ...
    if (!known.has(path)) throw httpError(403, 'not-produced-file', '只能删除该会话产出文件列表中的文件')
  }
  ...
  if (!allowed) throw httpError(403, 'outside-workspace', '只能删除工作区内的文件')
...
export async function openSessionFolder(ctx: PluginContext, sessionId: string): Promise<{ sessionId: string; path: string; opened: boolean }> {
  ...
  if (meta === undefined) throw httpError(404, 'session-not-found', '找不到该会话的记录目录（会话不存在）')
