...
import type { PluginContext, WorkspaceRegistry } from './types.js'

/** Archive one session into the registry-global archive set (public API). */
export async function archiveSession(ctx: PluginContext, sessionId: string): Promise<{ sessionId: string; archived: true }> {
  const registry = ctx.get<WorkspaceRegistry>('workspaceRegistry')
  if (registry === undefined || typeof registry.archiveSession !== 'function') {
    throw httpError(501, 'unsupported', '当前 Harness 版本不支持归档会话（缺少 workspaceRegistry.archiveSession）')
  }
...
export async function unarchiveSession(ctx: PluginContext, sessionId: string): Promise<{ sessionId: string; archived: false }> {
  const registry = ctx.get<WorkspaceRegistry>('workspaceRegistry')
  if (registry === undefined || typeof registry.unarchiveSession !== 'function') {
    throw httpError(501, 'unsupported', '当前 Harness 版本不支持取消归档（缺少 workspaceRegistry.unarchiveSession）')
  }
  const meta = await findSessionMeta(ctx, sessionId)
  if (meta === undefined) {
    throw httpError(404, 'session-not-found', '找不到该会话的记录（会话不存在）')
