...
export async function deleteFile(ctx, path, sessionId) {
    ...
    try {
        ...
        if (info.isDirectory())
            throw httpError(403, 'not-a-file', 'Only files can be deleted, not directories');
    }
    catch (error) {
    ...
    if (sessionId !== '') {
        ...
        if (!known.has(path))
            throw httpError(403, 'not-produced-file', 'Only files produced by this session can be deleted');
    }
    ...
    if (!allowed)
        throw httpError(403, 'outside-workspace', 'Only files inside a workspace can be deleted');
...
export async function openSessionFolder(ctx, sessionId) {
    ...
    if (meta === undefined)
        throw httpError(404, 'session-not-found', 'Session record folder not found (session does not exist)');
